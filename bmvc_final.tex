...
Each sample code is written by Python3 and only depends some standard modules for readability and easy to use.
...
It needs to know where am I (localization), where is safe (mapping), where and how to go (path planning), and how to control (path following). 
...
However, it might has an issue, because of lacking of documentations about it's basic algorithm.
Also, it might be written by a complicated programming language such as C++ for calculation performance and platform reasons.
This is not good for focusing on learning robotics algorithms.
...
It's learning strategy is great for beginners to understand it's technical background.
