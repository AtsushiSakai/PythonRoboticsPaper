...
\section{Philosophy}
\st{In this section, the philosophy of this project is described.
It}\jwd{The PythonRobotics project is} based on three main philosophies.  \jwd{Each of which will be discussed separately in this section.}

\jwd{I'M NOT SURE THAT THESE NEED TO BE SUBSECTIONS.  YOU COULD PROBABLY JUST USE BULLET POINTS}

\subsection{Readability}
The first \st{one}\jwd{philosophy} is that the code \st{have}\jwd{has} to be easy to read\jwd{.} \st{for understanding each algorithm.}
\st{This project aims beginners of robotics to understand basic ideas of each algorithm}\jwd{If the code is not easy to read, it would be difficult to achieve our goal of allowing beginners to understand the algorithms}. 
\st{Python programming language is adopted in this project because of its simplicity and it allows us to focus on algorithm itself.}\jwd{THIS SENTENCE DOESN'T MAKE ANY SENSE HERE.  PYTHON IS NOT MORE OR LESS EASY TO READ THAN ANY OTHER LANGUAGE.  I'D SUGGEST REMOVING THE SENTENCE ENTIRELY.}
Python has great libraries for matrix operation, mathematical and scientific operation, and visualization\st{.}\jwd{, which makes code more readable because such operations don't need to be reimplemented.  Having the core Python packages available allows the user to focus on the algorithms, rather than the implementations.}
\st{These libraries also allows us to focus on algorithm itself.}
...
These modules are OSS and \st{could}\jwd{can also} be used for free.
\st{This repository software}\jwd{PythonRobotics} doesn't depend on any commercial software.
...
\st{In this section, we briefly described the repository structure.}\jwd{THIS SENTENCE IS REDUNDANT}

\st{This repository has five directories which}\jwd{There are five directories, each one} corresponding to \st{five}\jwd{a different} technical categor\st{ies}\jwd{y} in autonomous navigation: localization, mapping, SLAM, path planning, and path tracking.
There is one sub-directory per algorithm, which has a sample code \st{using different algorithms}\jwd{implementing and testing the algorithm}.
...
Path planning is the ability of a robot \jwd{to find an allowable path from some starting point to a desired final point.  This typically means} \st{to} search\jwd{ing over} feasible \st{and efficient} path\jwd{s} to the goal \jwd{and choosing the ``best'' one}.
The \jwd{best} path ha\st{ve}\jwd{s} to satisfy some constraints based on \jwd{the robot's} \st{movement}\jwd{motion} model and obstacle positions\st{,}\jwd{.  By ``best'' path, it is typically meant that the path with highest reward (or lowest cost), with respect to some objective function, is chosen.} \st{and optimize some objective functions such as}\jwd{Some example objective functions include} time to goal and distance to obstacle\st{ etc}.
...
Path tracking is \st{an}\jwd{the} ability of a robot to follow the reference path generated by \jwd{a path planner}\st{ing algorithms}.
The role of the path tracking controller is to \st{stabilize to the reference path or trajectory which has}\jwd{follow a reference trajectory while simultaneously stabilizing the robot.  The path tracking controller may need to account for} modeling error and other forms of uncertainty.
